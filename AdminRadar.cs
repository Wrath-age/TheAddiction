    ...
    internal class AdminRadar : RustPlugin
    {
        ...
        private class Radar : FacepunchBehaviour
        {
            ...
            private void DirectDrawAll()
            {
                ...
                // Draw all cached entities for this radar instance.  A configurable limit
                // on the number of draw calls per update helps mitigate excessive packet
                // usage on busy servers.  When the limit is non‑zero the method stops
                // sending further draw commands after the limit has been reached, and any
                // remaining entities will be drawn on the next update.  This ensures the
                // radar stays responsive while not flooding the client.
                int drawCount = 0;
                int maxCalls = config.Settings.MaxDrawCallsPerUpdate;
                foreach (var pair in data)
                {
                    if (maxCalls > 0 && drawCount >= maxCalls)
                    {
                        break;
                    }
                    cobj = pair.Value;
                    ...
                    if (maxCalls > 0 && drawCount >= maxCalls) { break; }
                    ...
                    if (cobj.HasFlag(DrawFlags.Text))
                    {
                        ...
                        {
                            HandleException(pair.Key, ex);
                            continue;
                        }
                        if (cobj.ei.info == null)
                        {
                            continue;
                        }
            ...
            public void TryCacheOnlinePlayer(BasePlayer target)
            {
                ...
                // Skip dead players, excluded players, or those that bypass radar
                if (target.IsKilled() || exclude.Contains(target.userID) || !canBypassOverride && CanBypass(target.UserIDString))
                {
                    return;
                }

                // Ignore players that are currently invisible via a vanish plugin.  We use Interface.CallHook
                // rather than accessing the plugin directly so any vanish implementation that exposes
                // an IsVanished or IsInvisible hook will be respected.  This call will return null if no
                // plugin implements the hook.
                try
                {
                    object invisible = Interface.CallHook("IsVanished", target) ?? Interface.CallHook("IsInvisible", target);
                    if (invisible is bool b && b)
                    {
                        return;
                    }
                }
                ...
                {
                    // ignore any exceptions from the hook call
                }
        ...
        public class ConfigurationSettings
        {
            ...
            // How frequently the radar updates drawings in seconds. Lower values provide a smoother
            // display but will generate more network traffic. The default has been reduced
            // significantly to improve responsiveness over the original 5 second default.  A
            // reasonable minimum of 0.1 seconds is also enforced to prevent extremely high
            // update rates from overwhelming the server.  These defaults may be overridden in
            // the configuration file.
            [JsonProperty(PropertyName = "Default Refresh Time")]
            public float DefaultInvokeTime = 0.3f;

            // The smallest allowed refresh rate (seconds). A lower bound of 0.1 seconds
            // prevents abuse while still allowing very responsive updates.  Previously this was
            // set to one second which resulted in sluggish behaviour on busy servers.
            [JsonProperty(PropertyName = "Minimum Refresh Time")]
            public float MinInvokeTime = 0.1f;

            // Limits the number of draw commands that will be sent to a client each
            // update.  Sending too many draw calls every frame can saturate the network
            // connection and degrade performance.  Setting this value allows server
            // administrators to tune the radar’s packet intensity.  A value of 0 disables
            // the limit (default behaviour prior to this patch).  If capped, any
            // remaining entities will be drawn on subsequent updates.
            [JsonProperty(PropertyName = "Max Draw Calls Per Update")]
            public int MaxDrawCallsPerUpdate = 0;
