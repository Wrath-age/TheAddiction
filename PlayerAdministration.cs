...
#pragma warning restore IDE0044, CS0649
        ...
        private class CuiColor
        {
            ...
            // Colour scheme overrides to provide a dark grey background with orange‑red accents.
            // This updated palette modernizes the interface without harsh contrasts.
            public static readonly CuiColor Background = new CuiColor(35, 35, 35, 0.8f);
            public static readonly CuiColor BackgroundMedium = new CuiColor(45, 45, 45, 0.85f);
            public static readonly CuiColor BackgroundDark = new CuiColor(25, 25, 25, 0.95f);
            public static readonly CuiColor Button = new CuiColor(45, 45, 45, 1f);
            public static readonly CuiColor ButtonInactive = new CuiColor(100, 100, 100, 1f);
            public static readonly CuiColor ButtonDecline = new CuiColor(200, 0, 0, 1f);
            // Use an orange‑red tone for dangerous actions to stand out against the dark background.
            public static readonly CuiColor ButtonDanger = new CuiColor(255, 94, 77, 1f);
            public static readonly CuiColor ButtonWarning = new CuiColor(255, 165, 0, 1f);
            public static readonly CuiColor ButtonSuccess = new CuiColor(76, 175, 80, 1f);
            // White text for better readability on dark backgrounds.
            public static readonly CuiColor Text = new CuiColor(255, 255, 255, 1f);
            public static readonly CuiColor TextAlt = new CuiColor(230, 230, 230, 1f);
            public static readonly CuiColor TextTitle = new CuiColor(255, 94, 77, 1f);
...
#pragma warning restore CS0162
        ...
        private bool GetIsMuted(ref BasePlayer aPlayer) {
            // If the player object is null (offline), assume not muted.  BetterChatMute does not support
            // retrieving mute state for offline players via this method.
            if (aPlayer == null)
                return false;
        ...
        #region Steam Avatar Fallback
        /// <summary>
        /// Attempts to download the Steam avatar for the specified user directly from the Steam community XML page.
        /// This is used when the ImageLibrary plugin does not have the avatar cached and we need a fallback method.
        /// The result is passed to ImageLibrary.AddImage so the image can be stored and retrieved normally.
        /// </summary>
        /// <param name="steamId">The player's Steam64 ID.</param>
        private void RequestSteamAvatar(ulong steamId)
        {
            if (steamId == 0uL)
                return;
            ...
            webrequest.Enqueue(url, null, (code, response) =>
            {
                ...
                try
                {
                    string avatarUrl = ParseAvatarUrlFromXml(response);
                    if (!string.IsNullOrEmpty(avatarUrl))
                    {
                        // Queue the image for download by ImageLibrary.  Once downloaded it can be retrieved via GetImage.
        ...
        /// <summary>
        /// Parses the avatarFull URL from the provided Steam community XML profile content.
        /// </summary>
        /// <param name="xml">The XML string returned by Steam community.</param>
        /// <returns>The URL to the full avatar image, or an empty string if not found.</returns>
        private string ParseAvatarUrlFromXml(string xml)
        {
            if (string.IsNullOrEmpty(xml))
                return string.Empty;
            // Find the <avatarFull> tag and extract the URL inside the <![CDATA[ ... ]]> section
            int fullTagStart = xml.IndexOf("<avatarFull", StringComparison.OrdinalIgnoreCase);
            if (fullTagStart < 0)
                return string.Empty;
            // Look for the start of the CDATA section after the avatarFull tag
            int cdataStart = xml.IndexOf("<![CDATA[", fullTagStart, StringComparison.OrdinalIgnoreCase);
            if (cdataStart < 0)
                return string.Empty;
            cdataStart += "<![CDATA[".Length;
            int cdataEnd = xml.IndexOf("]]>", cdataStart, StringComparison.OrdinalIgnoreCase);
            if (cdataEnd < 0)
                return string.Empty;
            string url = xml.Substring(cdataStart, cdataEnd - cdataStart).Trim();
            return url;
        }

        /// <summary>
        /// Generate a fallback avatar image for a player.  When external avatar downloads fail (for example, due
        /// to firewall restrictions or missing API keys) this method creates a small identicon‑style graphic
        /// based on the player's Steam64 ID.  The identicon uses a deterministic colour derived from the
        /// player's ID and a simple pattern so that each player has a unique but consistent visual marker.
        /// </summary>
        /// <param name="steamId">The player's Steam64 ID.</param>
        /// <returns>A base64‑encoded PNG representation of the generated avatar.</returns>
        private string GenerateIdenticon(ulong steamId)
        {
            if (steamId == 0UL)
                return string.Empty;
            // Check cache to avoid regenerating avatars
            if (FGeneratedAvatars.TryGetValue(steamId, out string cached))
                return cached;
            // Determine a unique colour using the Steam ID bytes.  We use a simple hash by splitting
            // the 64‑bit ID into bytes and mapping them to RGB components.  The alpha channel is always full.
            byte[] idBytes = BitConverter.GetBytes(steamId);
            // Ensure the array is at least 3 bytes long (BitConverter returns 8 bytes on little endian platforms)
            byte r = idBytes.Length > 0 ? idBytes[0] : (byte)0;
            byte g = idBytes.Length > 1 ? idBytes[1] : (byte)0;
            byte b = idBytes.Length > 2 ? idBytes[2] : (byte)0;
            // Create a small square texture.  A 32x32 size is used to balance clarity and file size.
            const int size = 32;
            Texture2D tex = new Texture2D(size, size, TextureFormat.RGBA32, false);
            // Convert the colour bytes into float components
            Color baseColour = new Color32(r, g, b, 255);
            // Generate a simple mirrored pattern.  For each pixel, decide whether to fill it with the base
            // colour or a slightly darker shade based on the X and Y coordinates.  Mirroring across the
            // vertical axis produces a symmetrical identicon effect.  The pattern is derived from the player's
            // ID to ensure determinism: we use individual bits of the ID to decide block fill.
            for (int y = 0; y < size; y++)
            {
                for (int x = 0; x < size / 2; x++)
                {
                    // Determine whether this block should be coloured based on bits of the ID.  Shift by
                    // x+y to vary the pattern.  If the bit is set, use the base colour; otherwise use a
                    // darker shade.
                    bool bitSet = ((steamId >> ((x + y) % 64)) & 1UL) == 1UL;
                    Color pixelColour = bitSet ? baseColour : Color.Lerp(baseColour, Color.black, 0.3f);
                    tex.SetPixel(x, y, pixelColour);
                    // Mirror horizontally
                    tex.SetPixel(size - 1 - x, y, pixelColour);
                }
            }
            tex.Apply();
            // Encode texture to PNG and convert to base64 string
            byte[] bytes;
            try
            {
            ...
            {
                // Fallback: if EncodeToPNG fails, return empty string
            ...
            // Prepend the data URI prefix expected by Rust UI for inline PNG data
        ...
        private void AddUserPageInfoLabels(ref Cui aUIObj, string aParent, ulong aPlayerId, ref BasePlayer aPlayer) {
            // Determine whether the target player object exists.  Offline players will have a null aPlayer.
            bool playerExists = aPlayer != null;
            bool isConnected = playerExists && aPlayer.IsConnected;
            bool isSleeping = playerExists && aPlayer.IsSleeping();
            bool isAlive = playerExists && aPlayer.IsAlive();
            ...
            // Recover & pre-process user connection data (only when connected)
            if (isConnected)
            {
                try
                {
                    CpAddress = aPlayer.net?.connection?.ipaddress?.Split(':')[0] ?? CpAddress;
            ...
            aUIObj.AddLabel(
                aParent, CUserPageLblConnectLbAnchor, CUserPageLblConnectRtAnchor, CuiColor.TextAlt,
                string.Format(
                    lang.GetMessage("Connection Label Format", this, aUIObj.PlayerIdString),
                    (isConnected
            ...
            aUIObj.AddLabel(
                aParent, CUserPageLblSleepLbAnchor, CUserPageLblSleepRtAnchor, CuiColor.TextAlt,
                string.Format(
                    lang.GetMessage("Status Label Format", this, aUIObj.PlayerIdString),
                    (isSleeping
            ...
            if (VerifyPermission(aUIObj.PlayerId, CPermDetailInfo)) {
                // Flags: flying and mounted.  For offline players leave both values empty.
                string flagFlying = (playerExists && aPlayer.IsFlying ? lang.GetMessage("Flying Label Text", this, aUIObj.PlayerIdString) : string.Empty);
                string flagMounted = (playerExists && aPlayer.isMounted ? lang.GetMessage("Mounted Label Text", this, aUIObj.PlayerIdString) : string.Empty);
                aUIObj.AddLabel(
                    aParent, CUserPageLblFlagLbAnchor, CUserPageLblFlagRtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Flags Label Format", this, aUIObj.PlayerIdString), flagFlying, flagMounted),
                    string.Empty, 14, TextAnchor.MiddleLeft
                );
                // Player position and rotation.  If offline, show N/A
                string posText = playerExists ? aPlayer.ServerPosition.ToString() : "N/A";
                string rotText = playerExists ? aPlayer.GetNetworkRotation().ToString() : "N/A";
                aUIObj.AddLabel(
                    aParent, CUserPageLblPosLbAnchor, CUserPageLblPosRtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Position Label Format", this, aUIObj.PlayerIdString), posText), string.Empty, 14,
                    TextAnchor.MiddleLeft
                );
                aUIObj.AddLabel(
                    aParent, CUserPageLblRotLbAnchor, CUserPageLblRotRtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Rotation Label Format", this, aUIObj.PlayerIdString), rotText), string.Empty, 14,
                    TextAnchor.MiddleLeft
                );
                // Last admin cheat has been calculated above.  Even offline players can show the last cheat value (or 'Never').
                aUIObj.AddLabel(
                ...
                // Idle time: If offline, show N/A
                string idleTime = playerExists ? Math.Round(aPlayer.IdleTime, 2).ToString() : "N/A";
                aUIObj.AddLabel(
                    aParent, CUserPageLblIdleLbAnchor, CUserPageLblIdleRtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Idle Time Label Format", this, aUIObj.PlayerIdString), idleTime), string.Empty, 14,
                    TextAnchor.MiddleLeft
                );
                // Show the player's IP address if the viewer toggled it; otherwise provide a button to reveal it.
                bool showIp = FShowIp.ContainsKey(aUIObj.PlayerId) && FShowIp[aUIObj.PlayerId].Contains(aPlayerId);
                if (showIp)
                {
                    // Display the IP address and a copy icon.  Shrink the label width to make room for the copy icon
                    CuiPoint ipLblMin = CUserPageLblUsrNetAddrLbAnchor;
                    CuiPoint ipLblMax = new CuiPoint(0.80f, CUserPageLblUsrNetAddrRtAnchor.Y);
                    CuiPoint ipCopyMin = new CuiPoint(0.80f, CUserPageLblUsrNetAddrLbAnchor.Y);
                    CuiPoint ipCopyMax = new CuiPoint(0.83f, CUserPageLblUsrNetAddrRtAnchor.Y);
                    string ipLabelText = string.Format(lang.GetMessage("Player Address Label Format", this, aUIObj.PlayerIdString), CpAddress);
                    aUIObj.AddLabel(aParent, ipLblMin, ipLblMax, CuiColor.TextAlt, ipLabelText, string.Empty, 14, TextAnchor.MiddleLeft);
                    // Encode the IP for safe passage in the command.  Uri.EscapeDataString handles any special characters
                    string encodedIp = Uri.EscapeDataString(CpAddress);
                    // Replace the copy icon with a simple 'C' to ensure proper rendering
                    aUIObj.AddButton(aParent, ipCopyMin, ipCopyMax, CuiColor.Button, CuiColor.Text, "C", $"playeradministration.copyinfo {encodedIp}", string.Empty, string.Empty, 14, TextAnchor.MiddleCenter);
                }
                else
                {
                    // Display a button to reveal the IP address.  Clicking the button toggles visibility for this viewer
                    string ipButtonText = string.Format(lang.GetMessage("Player Address Label Format", this, aUIObj.PlayerIdString), lang.GetMessage("Click To Show Text", this, aUIObj.PlayerIdString));
                    aUIObj.AddButton(aParent, CUserPageLblUsrNetAddrLbAnchor, CUserPageLblUsrNetAddrRtAnchor, CuiColor.Button, CuiColor.Text, ipButtonText, $"playeradministration.showip {aPlayerId}", string.Empty, string.Empty, 14, TextAnchor.MiddleLeft);
                }
                // Always display the average ping label
                aUIObj.AddLabel(aParent, CUserPageLblUsrAvrPingLbAnchor, CUserPageLblUsrAvrPingRtAnchor, CuiColor.TextAlt, string.Format(lang.GetMessage("Player Ping Label Format", this, aUIObj.PlayerIdString), CpPing), string.Empty, 14, TextAnchor.MiddleLeft);
                ...
                // Display whether the target player has Godmode enabled.  Only call into the Godmode plugin if it exists and meets the minimum version requirement.
                if (Godmode != null && Godmode.Version.Major >= 4 && Godmode.Version.Minor >= 2 && Godmode.Version.Patch >= 9)
                {
                    bool isGod = false;
                    try
                    {
                        isGod = (bool)(Godmode.Call("IsGod", aPlayerId) ?? false);
                    }
                    catch { /* ignore errors calling Godmode */ }
                    // Convert the boolean into a string for display.  Rust's bool.ToString() will output "True" or "False".
                    string godStatus = isGod.ToString();
                    aUIObj.AddLabel(
                        aParent, CUserPageLblGodmodeLbAnchor, CUserPageLblGodmodeRtAnchor, CuiColor.TextAlt,
                        string.Format(lang.GetMessage("Godmode Status Label Format", this, aUIObj.PlayerIdString), godStatus), string.Empty, 14,
                        TextAnchor.MiddleLeft
                    );
                }
                else
                {
                    // If the Godmode plugin is missing or outdated, display N/A
                    aUIObj.AddLabel(
                        aParent, CUserPageLblGodmodeLbAnchor, CUserPageLblGodmodeRtAnchor, CuiColor.TextAlt,
            ...
            LogDebug("AddUserPageInfoLabels > Godmode info has been added.");
                // Player statistics.  Provide 'N/A' for offline players.
                string healthVal = playerExists ? aPlayer.health.ToString() : "N/A";
                string calVal = playerExists && aPlayer.metabolism?.calories != null ? aPlayer.metabolism.calories.value.ToString() : "N/A";
                string hydraVal = playerExists && aPlayer.metabolism?.hydration != null ? aPlayer.metabolism.hydration.value.ToString() : "N/A";
                string tempVal = playerExists && aPlayer.metabolism?.temperature != null ? aPlayer.metabolism.temperature.value.ToString() : "N/A";
                string wetVal = playerExists && aPlayer.metabolism?.wetness != null ? aPlayer.metabolism.wetness.value.ToString() : "N/A";
                string comfortVal = playerExists && aPlayer.metabolism?.comfort != null ? (100 * aPlayer.metabolism.comfort.value).ToString() : "N/A";
                string bleedVal = playerExists && aPlayer.metabolism?.bleeding != null ? aPlayer.metabolism.bleeding.value.ToString() : "N/A";
                string rads1Val = playerExists && aPlayer.metabolism?.radiation_poison != null ? aPlayer.metabolism.radiation_poison.value.ToString() : "N/A";
                string rads2Val = playerExists ? aPlayer.RadiationProtection().ToString() : "N/A";
                aUIObj.AddLabel(
                    aParent, CUserPageLblHealthLbAnchor, CUserPageLblHealthRtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Health Label Format", this, aUIObj.PlayerIdString), healthVal), string.Empty, 14, TextAnchor.MiddleLeft
                );
                aUIObj.AddLabel(
                    aParent, CUserPageLblCalLbAnchor, CUserPageLblCalRtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Calories Label Format", this, aUIObj.PlayerIdString), calVal), string.Empty,
                    14, TextAnchor.MiddleLeft
                );
                aUIObj.AddLabel(
                    aParent, CUserPageLblHydraLbAnchor, CUserPageLblHydraRtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Hydration Label Format", this, aUIObj.PlayerIdString), hydraVal), string.Empty,
                    14, TextAnchor.MiddleLeft
                );
                aUIObj.AddLabel(
                    aParent, CUserPageLblTempLbAnchor, CUserPageLblTempRtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Temp Label Format", this, aUIObj.PlayerIdString), tempVal), string.Empty, 14,
                    TextAnchor.MiddleLeft
                );
                aUIObj.AddLabel(
                    aParent, CUserPageLblWetLbAnchor, CUserPageLblWetRtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Wetness Label Format", this, aUIObj.PlayerIdString), wetVal), string.Empty, 14,
                    TextAnchor.MiddleLeft
                );
                aUIObj.AddLabel(
                    aParent, CUserPageLblComfortLbAnchor, CUserPageLblComfortRtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Comfort Label Format", this, aUIObj.PlayerIdString), comfortVal), string.Empty, 14,
                    TextAnchor.MiddleLeft
                );
                aUIObj.AddLabel(
                    aParent, CUserPageLblBleedLbAnchor, CUserPageLblBleedRtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Bleeding Label Format", this, aUIObj.PlayerIdString), bleedVal), string.Empty,
                    14, TextAnchor.MiddleLeft
                );
                aUIObj.AddLabel(
                    aParent, CUserPageLblRads1LbAnchor, CUserPageLblRads1RtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Radiation Label Format", this, aUIObj.PlayerIdString), rads1Val),
                    string.Empty, 14, TextAnchor.MiddleLeft
                );
                aUIObj.AddLabel(
                    aParent, CUserPageLblRads2LbAnchor, CUserPageLblRads2RtAnchor, CuiColor.TextAlt,
                    string.Format(lang.GetMessage("Radiation Protection Label Format", this, aUIObj.PlayerIdString), rads2Val),
                    string.Empty, 14, TextAnchor.MiddleLeft
                );
        ...
        private void BuildUserPage(ref Cui aUIObj, UiPage aPageType, ulong aPlayerId) {
            ...
            if (aPageType == UiPage.PlayerPage) {
                ...
                if (aPlayerId != aUIObj.PlayerId && VerifyPermission(aUIObj.PlayerId, CPermMute)) {
                    bool isMuted = false;
                    if (player != null)
                    {
                        try { isMuted = GetIsMuted(ref player); } catch { isMuted = false; }
                    }
                    if (isMuted) {
                        aUIObj.AddButton(
        ...
        private void BuildUI(BasePlayer aPlayer, UiPage aPageType, string aArg = "", bool aIndFiltered = false) {
            ...
            // For player pages start a repeating refresh timer to keep the player info panel
            // up‑to‑date.  The timer will rebuild the UI at the configured interval.  To
            // avoid spamming timers we only create one per admin; if a timer already exists it
            // will not be recreated.  When the admin switches away from the player page or
            // closes the UI the timer is destroyed in the close and switch callbacks.
            if (aPageType == UiPage.PlayerPage)
            {
                StartUserPageRefresh(aPlayer, aPageType, aArg);
            }
        }
        ...
        // Tracks which admins have chosen to reveal the IP address of a given player on the UI.  The key is the admin's user ID
        // and the value is a set of player IDs whose IP addresses should be shown.  This allows per‑viewer toggling of sensitive
        // data without exposing it to other players.
        private readonly Dictionary<ulong, HashSet<ulong>> FShowIp = new Dictionary<ulong, HashSet<ulong>>();
        // Tracks which Steam avatars have already been requested via manual web request.  This ensures we do not
        // repeatedly fetch the same avatar from SteamCommunity when using the fallback XML parser.
        private readonly HashSet<ulong> FRequestedAvatars = new HashSet<ulong>();
        // Timers used to refresh the player info panel on the player page.  The key is the admin
        // userID viewing the page, the value is a repeating timer.  When a player page is open
        // this timer will periodically rebuild the UI to update the target player's stats.  The
        // timer is destroyed when the admin closes or navigates away from the player page.
        private readonly Dictionary<ulong, Timer> FUserPageRefreshTimers = new Dictionary<ulong, Timer>();
...
#pragma warning disable IDE0051, IDE0060 // Remove unused private members, unused parameter
        ...
        void OnServerInitialized() {
            ...
            // Include all known users in the offline list.  The standard implementation only adds sleepers,
            // which means players who disconnect without leaving a sleeper will never appear here.
            // To build a complete offline list we iterate through each user group
            // (Owner, Moderator and None) in the server users file, skipping banned users.  Any
            // user not already in the online list is placed into the offline list.
            try {
                foreach (var su in ServerUsers.GetAll(ServerUsers.UserGroup.Owner)) {
                    if (!FOnlineUserList.ContainsKey(su.steamid) && !FOfflineUserList.ContainsKey(su.steamid)) {
                        FOfflineUserList[su.steamid] = su.username;
                    }
                }
                foreach (var su in ServerUsers.GetAll(ServerUsers.UserGroup.Moderator)) {
                    if (!FOnlineUserList.ContainsKey(su.steamid) && !FOfflineUserList.ContainsKey(su.steamid)) {
                        FOfflineUserList[su.steamid] = su.username;
                    }
                }
                foreach (var su in ServerUsers.GetAll(ServerUsers.UserGroup.None)) {
                    if (!FOnlineUserList.ContainsKey(su.steamid) && !FOfflineUserList.ContainsKey(su.steamid)) {
                        FOfflineUserList[su.steamid] = su.username;
                    }
                }
        ...
        protected override void LoadConfig() {
            ...
            // After loading and upgrading the configuration, apply runtime settings
            // Apply the configured refresh interval, clamping to a minimum of 0.25 seconds to prevent extreme refresh rates
            if (FConfigData != null)
            {
                float interval = FConfigData.UserPageRefreshInterval;
                if (interval < 0.25f)
                    interval = 0.25f;
                FUserPageRefreshInterval = interval;
            }
        ...
        protected override void LoadDefaultMessages() {
            ...
            { "Click To Show Text", "Click to show" },
        ...
        void PlayerAdministrationCloseUICallback(IPlayer aPlayer, string aCommand, string[] aArgs) {
            ...
            BasePlayer player = BasePlayer.Find(aPlayer.Id);
            // Stop any active player page refresh timer for this admin
            StopUserPageRefresh(player.userID);
        ...
        void PlayerAdministrationSwitchUICallback(IPlayer aPlayer, string aCommand, string[] aArgs) {
            ...
            // If switching pages, stop any active refresh timer to avoid multiple timers
            if (player != null)
                StopUserPageRefresh(player.userID);
        ...
        /// <summary>
        /// Handles clicks on the "Show IP" button.  When invoked the viewer
        /// receives a chat message containing the target player's IP address.  The
        /// IP is not displayed in the UI to protect sensitive data.
        /// </summary>
        /// <param name="aPlayer">The player who clicked the button.</param>
        ...
        void PlayerAdministrationShowIp(IPlayer aPlayer, string aCommand, string[] aArgs)
        {
            ...
            BasePlayer player = BasePlayer.Find(aPlayer.Id);
            // Ensure the viewer still has permission to view detailed info
            ...
            // Toggle IP visibility for this viewer/target pair.  When on, the IP will appear in the UI
            if (!FShowIp.ContainsKey(player.userID))
                FShowIp[player.userID] = new HashSet<ulong>();
            if (FShowIp[player.userID].Contains(targetId))
                FShowIp[player.userID].Remove(targetId);
            else
                FShowIp[player.userID].Add(targetId);
        ...
        /// <summary>
        /// Handles clicks on the copy‑info buttons.  When invoked the value passed in the arguments is
        /// unescaped and sent back to the requesting admin via chat, so they can easily copy it.  This
        /// method allows the UI to include small copy icons beside fields like the SteamID or IP address.
        /// </summary>
        /// <param name="aPlayer">The player who clicked the button.</param>
        /// <param name="aCommand">The invoked command name.</param>
            ...
            timer.Once(0.1f, () => BuildUI(player, pageType, targetId.ToString()));
        {
            ...
            BasePlayer player = BasePlayer.Find(aPlayer.Id);
            // Basic validation: ensure the player exists and has permission to interact with the UI
            if (player == null || !VerifyPermission(ref player, string.Empty, true))
                return;
            ...
            try
            {
                // The first argument is the encoded value (such as a SteamID or IP).  Join all arguments to allow spaces
                string encoded = string.Join(" ", aArgs);
                string decoded = System.Uri.UnescapeDataString(encoded);
                // Send the decoded value directly to the player via chat
                player.ChatMessage(decoded);
            }
            catch { }
        }
        ...
        #region Player Page Refresh
        /// <summary>
        /// Starts a repeating refresh timer for the player page.  When the specified admin is
        /// viewing a player page this will rebuild the UI at the configured interval to keep
        /// the target player's stats up‑to‑date.  Only one timer is created per admin; if a
        /// timer already exists it will not be recreated.  Timers are destroyed when the
        /// admin closes the UI or navigates away from the player page.
        /// </summary>
        ...
        void StartUserPageRefresh(BasePlayer admin, UiPage pageType, string playerArg)
        {
            ...
            // Only start timers for the player page.  Other pages are static and do not need
            // periodic updates.
            if (pageType != UiPage.PlayerPage)
                return;

            // Do not create a second timer if one is already running for this admin
            if (FUserPageRefreshTimers.ContainsKey(admin.userID))
                return;

            FUserPageRefreshTimers[admin.userID] = timer.Every(FUserPageRefreshInterval, () =>
            {
                // If the admin disconnects stop refreshing
                if (admin == null || !admin.IsConnected)
                {
                    StopUserPageRefresh(admin?.userID ?? 0);
                    return;
                }

                // Rebuild the player page.  Note that BuildUI will detect that a refresh timer
                // already exists and will not attempt to start another one.
                BuildUI(admin, pageType, playerArg);
            });
        }

        /// <summary>
        /// Stops the refresh timer for the specified admin user.  Called when the admin
        ...
        void StopUserPageRefresh(ulong adminId)
        {
            ...
            Timer t;
            if (FUserPageRefreshTimers.TryGetValue(adminId, out t))
            {
                t.Destroy();
                FUserPageRefreshTimers.Remove(adminId);
            }
        }
        #endregion
