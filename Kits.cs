...
namespace Oxide.Plugins
{
    ...
    class Kits : RustPlugin
    {
        ...
        private void OpenKitGrid(BasePlayer player, int page = 0, ulong npcId = 0UL)
        {
            ...
            // Use a gradient background for the header bar
            UI.GradientPanel(container, UI_MENU, Configuration.Menu.Color3.Get, Configuration.Menu.Color4.Get, new UI4(0.005f, 0.93f, 0.995f, 0.99f));
        ...
        {            
            ...
            // Kit name header uses a gradient
            UI.GradientPanel(container, UI_MENU, Configuration.Menu.Color3.Get, Configuration.Menu.Color4.Get, new UI4(position.xMin, position.yMax, position.xMax, position.yMax + 0.04f));
        ...
        private void OpenKitView(BasePlayer player, string name, int page, ulong npcId)
        {
            ...
            // Use gradient for the header bar in kit view
            UI.GradientPanel(container, UI_MENU, Configuration.Menu.Color3.Get, Configuration.Menu.Color4.Get, new UI4(0.005f, 0.93f, 0.995f, 0.99f));
        ...
        // Draws a simple vertical gradient by splitting the rectangle into two halves
        public static void GradientPanel(CuiElementContainer container, string panel, string colorTop, string colorBottom, UI4 dimensions)
        {
            // Calculate mid-point of the Y-axis to split the panel vertically
            float yMid = dimensions.yMin + (dimensions.yMax - dimensions.yMin) * 0.5f;
            // Top half uses the top color
            Panel(container, panel, colorTop, new UI4(dimensions.xMin, yMid, dimensions.xMax, dimensions.yMax));
            // Bottom half uses the bottom color
            Panel(container, panel, colorBottom, new UI4(dimensions.xMin, dimensions.yMin, dimensions.xMax, yMid));
        }
